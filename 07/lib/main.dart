...
class DataService {
  ...
  void loadData(int index, int? itemCount) async {
    if (index == 0) {
      await loadCoffees(itemCount);
    } else if (index == 1) {
      await loadBeers(itemCount);
    } else if (index == 2) {
      await loadNations(itemCount);
    }
  }

  Future<void> loadCoffees(int? itemCount) async {
    var coffeeUri = Uri(
      scheme: 'https',
  ...
  Future<void> loadBeers(int? itemCount) async {
    var beersUri = Uri(
      scheme: 'https',
      host: 'random-data-api.com',
  ...
  Future<void> loadNations(int? itemCount) async {
    var nationsUri = Uri(
      scheme: 'https',
      host: 'random-data-api.com',
...
class _MyAppState extends State<MyApp> {
  ...
  void initState() {
    super.initState();
    dataService.loadData(_selectedIndex, _itemCount);
  }
